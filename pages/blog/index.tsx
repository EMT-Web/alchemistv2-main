...
const localPostsAsCards = localBlogPosts.map((p) => ({
  _id: `local-${p.slug}`,
...
export default function  NextPage({ posts: sanityPosts}:any) {
  const router = useRouter();
  const posts = [...(sanityPosts || []), ...localPostsAsCards].sort(
  ...
  const blogCollectionSchema = {
    ...
    "blogPost": posts?.slice(0, 10).map((post: any) => ({
      ...
      "image": post.mainImage ? urlFor(post.mainImage).url()! : post.localImage
    }))
  };
