...
function destinationDetails({ destination, destinations, relatedTours}:any) {
  ...
  const breadcrumbSchema = createBreadcrumbSchema([
    ...
    { name: destination.city, url: `/destinations/${destination.slug.current}` }
  ]);
  ...
  const placeSchema = {
    ...
    "description": destination.seodescription || destination.heroparagraph,
    "url": `https://www.escortedmoroccotours.com/destinations/${destination.slug.current}`,
    "image": destination.mainImage ? urlFor(destination.mainImage).url()! : undefined,
      ...
      <div className="col-lg-4 sidebar ftco-animate bg-light py-md-5">
        ...
        <div className="sidebar-box ftco-animate">
          <div className="categories">
            <h3>Destinations</h3>
            { destinations! && destinations.map((d:any)=> <li><p><a href={`/destinations/${d.slug.current}`} target="_blank">{d.city} </a><a href={`/destinations/${d.slug.current}`}><span>{`(${d.count} Tours)`}</span></a></p></li> )}
